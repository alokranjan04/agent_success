...
function analyzeConversation(messages: { role: string; text: string }[]) {
    ...
    if (messages.length === 0) {
        ...
        // Check if contact info has been shared anywhere in conversation
        ...
        // If contact details haven't been shared yet, prioritize validation replies
                        ...
                        {conversations.length === 0 ? (
                            ...
                            conversations.map(conv => (
                                <div
                                    ...
                                    className={`mx-2 mb-1 p-3 rounded-xl cursor-pointer transition-all border ${activeConvId === conv.id ? 'bg-white border-blue-200 shadow-md ring-1 ring-blue-50' : 'bg-transparent border-transparent hover:bg-slate-200/50'}`}
                                >
                                    <div className="flex items-center gap-3">
                                        <div className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs shadow-sm ${activeConvId === conv.id ? 'bg-blue-600 text-white' : 'bg-white text-slate-400 border border-slate-200'}`}>
                                            {conv.customerInfo?.name?.charAt(0) || 'C'}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-[10px] text-slate-400 truncate mt-0.5">
                                                {conv.messages.length > 0 ? conv.messages[conv.messages.length - 1].text : 'New chat'}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div key={i} className="flex gap-4 group/msg">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between mb-1.5 px-1">
                                                        <span className={`text-[10px] font-bold uppercase tracking-wider ${m.role === 'agent' ? 'text-blue-500' : 'text-slate-400'}`}>
                                                            {m.role === 'agent' ? 'Agent' : 'Customer'}
                                                        </span>
                                                        <button onClick={() => handlePlayTTS(m.text, i)} className="text-slate-200 hover:text-blue-500 transition-colors">
