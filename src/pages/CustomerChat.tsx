...
const CustomerChat: React.FC = () => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 font-sans p-4">
            {/* Chat Window */}
            <div className="w-full max-w-lg flex flex-col bg-white rounded-3xl shadow-2xl shadow-blue-200/40 overflow-hidden border border-slate-200/60" style={{ height: '85vh' }}>
                {/* Header */}
                <header className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-6 py-4 shrink-0 relative overflow-hidden">
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <a href="/" className="p-1 -ml-1 rounded-lg hover:bg-white/10 transition-colors" title="Switch to Agent View">
                            ...
                            <div>
                                <h1 className="font-bold text-sm tracking-tight">Support Chat</h1>
                                <div className="flex items-center gap-1.5 mt-0.5">
                                    <span className={`w-1.5 h-1.5 rounded-full ${agentOnline ? 'bg-emerald-400 animate-pulse shadow-[0_0_6px_rgba(52,211,153,0.6)]' : 'bg-slate-400'}`}></span>
                                    <span className="text-[10px] text-blue-100 font-medium">{agentOnline ? 'Agent is online' : 'Connecting...'}</span>
                                </div>
                            </div>
                        </div>
                        <div className="flex items-center gap-2">
                            <span className="text-[10px] bg-white/15 px-3 py-1 rounded-full font-bold tracking-wide backdrop-blur-sm">Live Chat</span>
                        </div>
                    </div>
                </header>

                {/* Messages */}
                <div className="flex-1 overflow-y-auto px-5 py-6 space-y-5 bg-gradient-to-b from-white to-slate-50/50">
                    {/* Empty state */}
                    {messages.length === 0 && !agentTyping && (
                        <div className="flex flex-col items-center justify-center h-full text-center">
                            ...
                            <p className="text-xs text-slate-300 mt-1">Type a message below to connect with an agent</p>
                        </div>
                    )}
                    ...
                    {messages.map((m, i) => (
                        <div key={m.id || i} className={`flex ${m.role === 'customer' ? 'justify-end' : 'justify-start'} items-end gap-2`}>
                            {m.role === 'agent' && (
                                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shrink-0 shadow-lg shadow-blue-200/50 mb-1">
                            ...
                            <div className="flex flex-col gap-1 max-w-[75%]">
                                <div
                                    className={`px-4 py-3 text-[13px] leading-relaxed ${m.role === 'customer'
                                        ? 'bg-gradient-to-br from-blue-600 to-indigo-600 text-white rounded-2xl rounded-br-md shadow-lg shadow-blue-200/40'
                                        : 'bg-white text-slate-700 rounded-2xl rounded-bl-md shadow-md shadow-slate-100 ring-1 ring-slate-100'
                                        }`}
                                >
                                    {m.text}
                                </div>
                                <span className={`text-[9px] font-bold text-slate-300 ${m.role === 'customer' ? 'text-right pr-1' : 'pl-1'}`}>
                                    {m.time}
                                </span>
                            </div>
                        </div>
                    ...
                    {agentTyping && (
                        <div className="flex items-end gap-2">
                            <div className="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center shrink-0 shadow-lg shadow-blue-200/50">
                                <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M18.364 5.636l-3.536 3.536m0 5.656l3.536 3.536M9.172 9.172L5.636 5.636m3.536 9.192l-3.536 3.536M21 12a9 9 0 11-18 0 9 9 0 0118 0zm-5 0a4 4 0 11-8 0 4 4 0 018 0z" />
                                </svg>
                            </div>
                            <div className="bg-white px-5 py-3 rounded-2xl rounded-bl-md shadow-md ring-1 ring-slate-100">
                ...
                <div className="px-5 py-2 bg-white border-t border-slate-100 shrink-0">
                    <div className="flex gap-2 overflow-x-auto pb-1">
                        <button
                            onClick={() => handleInputChange('What is the status of my refund?')}
                            className="shrink-0 text-[10px] font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-full border border-blue-100 transition-colors"
                        >
                            Check refund status
                        </button>
                        <button
                            onClick={() => handleInputChange('I want to speak with a supervisor.')}
                        ...
                        <button
                            onClick={() => handleInputChange('Can you email me a summary of this conversation?')}
                            className="shrink-0 text-[10px] font-bold text-slate-500 bg-slate-50 hover:bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 transition-colors"
                        >
                            Email summary
                        </button>
                ...
                {/* Input Area */}
                <footer className="px-4 py-4 bg-white border-t border-slate-200 shrink-0">
                    <div className="flex items-center gap-2">
                        <button className="p-2 rounded-xl text-slate-400 hover:text-blue-500 hover:bg-blue-50 transition-colors shrink-0">
                            <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                            </svg>
                        </button>
                        <input
                            className="flex-1 bg-slate-100 border-0 rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-200 transition-all placeholder:text-slate-400 font-medium"
                ...
                <div className="text-center py-2 bg-slate-50 border-t border-slate-100">
                    <span className="text-[9px] text-slate-400 font-medium">Powered by <strong className="text-blue-500">{companyName}</strong> • Live Chat</span>
                </div>
            </div>
        </div>
    )
