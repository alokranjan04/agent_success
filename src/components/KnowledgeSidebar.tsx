...
const KnowledgeSidebar: React.FC<KnowledgeSidebarProps> = ({ autoSnippets, apiUrl }) => {
    ...
    return (
        <div className="flex flex-col h-full gap-5 animate-in fade-in slide-in-from-right duration-500">
            {/* Manual Search */}
            <section className="shrink-0">
                <div className="flex items-center justify-between mb-2">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Manual Search</h3>
                </div>
                <form onSubmit={handleSearch} className="relative group">
                    <input
                        ...
                        className="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all shadow-sm"
                    />
                    <button
                        type="submit"
                        disabled={isSearching || !searchQuery.trim()}
                        className="absolute right-2 top-1.5 p-1 text-slate-400 hover:text-indigo-600 disabled:opacity-30 transition-colors"
                    >
                        {isSearching ? (
            ...
            {searchResults.length > 0 && (
                <section className="animate-in fade-in slide-in-from-top-2 duration-300">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="text-[10px] font-black uppercase tracking-widest text-indigo-500">Search Results</h3>
                        <button
                        ...
                        {searchResults.map((snippet, i) => (
                            <div key={`search-${i}`} className="bg-white border border-indigo-50 rounded-xl p-3 text-[11px] leading-relaxed shadow-sm hover:shadow-md transition-shadow relative group">
                                <div className="flex items-center justify-between mb-1.5">
                                    <span className="text-[9px] font-black uppercase text-indigo-400 px-1.5 py-0.5 bg-indigo-50 rounded">
                                        {snippet.docName}
                                    </span>
                                    <button
            ...
            <section className="flex-1 overflow-y-auto min-h-0 pr-1 custom-scrollbar">
                <div className="flex items-center justify-between mb-3 sticky top-0 bg-slate-50 pb-1 z-10">
                    <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400">Live Suggestions</h3>
                    <div className="flex items-center gap-1 px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600 text-[9px] font-black animate-pulse">
                ...
                {autoSnippets.length === 0 ? (
                    <div className="bg-white/50 border border-dashed border-slate-200 rounded-2xl p-8 text-center">
                        ...
                        <p className="text-[11px] text-slate-400 font-medium">Listening for keywords to suggest relevant knowledge...</p>
                    </div>
                ) : (
                    <div className="space-y-3">
                            ...
                            <div key={`auto-${i}`} className="bg-gradient-to-br from-emerald-50/50 to-white border border-emerald-100 rounded-2xl p-4 text-[11px] leading-relaxed shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group relative overflow-hidden">
                                ...
                                <div className="flex items-center justify-between mb-2 relative z-10">
                                    <span className="text-[9px] font-black uppercase text-emerald-600 flex items-center gap-1">
                                        <svg className="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20"><path d="M9 4.804A7.968 7.968 0 005.5 4c-1.255 0-2.434.29-3.48.804v10a7.969 7.969 0 013.48-.804c1.336 0 2.59.322 3.696.895 1.106-.573 2.36-.895 3.696-.895a7.969 7.969 0 013.48.804v-10A7.968 7.968 0 0014.5 4c-1.255 0-2.434.29-3.48.804V4.804z" /></svg>
                                        {snippet.docName}
                                    </span>
                                    <button
                                        onClick={() => copyToClipboard(snippet.text, i)}
                                        className="opacity-0 group-hover:opacity-100 text-[10px] font-black text-emerald-700 underline decoration-emerald-300 underline-offset-2 transition-opacity"
                                    >
